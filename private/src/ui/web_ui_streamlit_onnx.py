...
model_path = f"../../generated/{model_name}/weights/best.onnx"
...
def main():
    ...
    threshold = st.slider(label="detection threshold", min_value=0.001, max_value=0.5, value=0.1, step=0.001, format="%0.3f")
...
# @st.experimental_memo
def predict(_image):
    return model.predict(_image, device="cpu", conf=0.001)[0].summary()
