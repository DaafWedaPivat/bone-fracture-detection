...
def _():
    ...
    random.seed(1)
    return glob, mo, os, random, shutil, cv2, np
...
def _():
    ...
    yolo_dataset = generated + "yolo_dataset_enhanced/"
    return (
        dataset,
        dataset_annotations,
        dataset_images,
...
def _(
    ...
    for image in fractured_train_images:
        process_image(dataset_images + "Fractured/" + image, yolo_dataset + "train/images/" + image)
    ...
    for image in non_fractured_train_images:
        process_image(dataset_images + "Non_fractured/" + image, yolo_dataset + "train/images/" + image)
        label = get_annotation_file(image)
        shutil.copyfile(dataset_annotations + label, yolo_dataset + "train/labels/" + label)

    for image in fractured_valid_images:
        process_image(dataset_images + "Fractured/" + image, yolo_dataset + "valid/images/" + image)
        label = get_annotation_file(image)
        shutil.copyfile(dataset_annotations + label, yolo_dataset + "valid/labels/" + label)

    for image in non_fractured_valid_images:
        process_image(dataset_images + "Non_fractured/" + image, yolo_dataset + "valid/images/" + image)
        label = get_annotation_file(image)
        shutil.copyfile(dataset_annotations + label, yolo_dataset + "valid/labels/" + label)

    for image in fractured_test_images:
        process_image(dataset_images + "Fractured/" + image, yolo_dataset + "test/images/" + image)
        label = get_annotation_file(image)
        shutil.copyfile(dataset_annotations + label, yolo_dataset + "test/labels/" + label)

    for image in non_fractured_test_images:
        process_image(dataset_images + "Non_fractured/" + image, yolo_dataset + "test/images/" + image)
        label = get_annotation_file(image)
        shutil.copyfile(dataset_annotations + label, yolo_dataset + "test/labels/" + label)
